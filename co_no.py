...
for query_idx, query in enumerate(QUERIES, 1):
    ...
    while page <= 8:
        ...
        page += 1
        time.sleep(0.6)
    print(f"[{datetime.now(beijing_tz).strftime('%H:%M:%S')}]   └─ 本关键词贡献 {query_links_count} 条链接")
...
if unique_nodes:
    with open("no.txt", "w", encoding="utf-8") as f:
        f.write("\n".join(unique_nodes))
    print(f"[{datetime.now(beijing_tz).strftime('%H:%M:%S')}] ✅ 已将 {len(unique_nodes)} 条去重后的节点保存到 no.txt")
