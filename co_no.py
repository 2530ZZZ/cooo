    ...
    for attempt in range(1, max_retries + 1):
        try:
            ...
            if resp.status_code == 409:
                print(f"[{datetime.now(beijing_tz).strftime('%H:%M:%S')}] {operation_name} 返回 409（仓库可能为空或无commit），快速跳过", flush=True)
                return None
...
def process_file_tree(repo, path="", branch="main"):
    ...
    if c_resp is None or c_resp.status_code != 200:
        print(f" [{datetime.now(beijing_tz).strftime('%H:%M:%S')}] Contents API 请求失败或超时: https://github.com/{repo}", flush=True)
        return
    ...
    for item in items:
        ...
        # 如果是文件 → 处理订阅文件
            ...
            #print(f" 🔄 处理订阅文件: {file_url}", flush=True)   # 显示完整 raw 链接

            # 直接请求一次文件内容，然后提取节点
            resp = safe_get(file_url, timeout=(10, 60), operation_name="获取订阅文件内容")
