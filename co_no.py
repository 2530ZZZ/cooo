...
@timeout_decorator(60)  # 【新增】单个仓库处理总时间不得超过60秒，防止网络死锁
...
def process_file_tree(repo, path="", branch="main", has_nodes=None):
    ...
    print(f" [{datetime.now(beijing_tz).strftime('%H:%M:%S')}] 进入目录: {current_path} | 仓库: https://github.com/{repo}", flush=True)
    ...
    for item in items:
        ...
        if f_resp is None or f_resp.status_code != 200:
            # 【新增】打印跳过原因，便于调试
            print(f"   ⏭️ 跳过 {full_item_path}：commit 查询失败 (状态码: {f_resp.status_code if f_resp else 'None'})", flush=True)
            continue
        ...
        try:
            ...
            if datetime.now(timezone.utc) - file_time >= timedelta(hours=24):
                # print(f"   ⏭️ 跳过 {full_item_path}：最后更新超过 24 小时", flush=True)
                continue


        except Exception as e:
            #print(f" [{datetime.now(beijing_tz).strftime('%H:%M:%S')}]  ⏭️ 跳过  路径 {full_item_path} 日期解析异常: {e}（已跳过）", flush=True)
            continue
